 ...
 *
 * This basically just checks for a E_Token::CT_COMPARE that isn't surrounded by parens.
 * The edges for the compare are the open, close and any E_Token::CT_BOOL tokens.
 *
...
void do_parens()
{
   ...
   if (options::mod_full_paren_if_bool())
   {
      ...
      {
         if (  pc->IsNot(E_Token::CT_SPAREN_OPEN)
         ...
         Chunk *pclose = pc->GetNextType(E_Token::CT_SPAREN_CLOSE, pc->GetLevel(), E_Scope::PREPROC);
      ...
      while ((pc = pc->GetNextNcNnl())->IsNotNullChunk())
      {
         if (pc->Is(E_Token::CT_ASSIGN))
         {
            ...
            while (p->IsNotNullChunk())
            {
               ...
               if (p->Is(E_Token::CT_PAREN_OPEN))
               {
                  check_level--;
               }

               if (p->Is(E_Token::CT_SPAREN_OPEN))
               {
                  break;
               }
            ...
            if (p->GetParentType() == E_Token::CT_WHILE)
            {
               continue;
            }
            ...
            if (pc->TestFlags(PCF_IN_ENUM))            // Issue #4191
            {
               ...
               Chunk *found_comma = pc->GetNextType(E_Token::CT_COMMA, pc->GetLevel());
               ...
               else
               {
                  ...
                  Chunk *found_brace = pc->GetNextType(E_Token::CT_BRACE_CLOSE, pc->GetLevel() - 1);
            ...
            else
            {
               ...
               Chunk *semicolon = pc->GetNextType(E_Token::CT_SEMICOLON, pc->GetLevel(), E_Scope::PREPROC);
...
static void add_parens_between(Chunk const *first, Chunk *last)
{
   ...
   pc.SetType(E_Token::CT_PAREN_OPEN);
   ...
   pc.SetType(E_Token::CT_PAREN_CLOSE);
...
static void check_bool_parens(Chunk *popen, Chunk *pclose, int nest)
{
         ...
         && pc != pclose)
   {
      ...
      else if (pc->Is(E_Token::CT_SEMICOLON))                      // Issue #3236
      {
