...
void fix_casts(Chunk *start)
{
   ...
   if (prev->Is(E_Token::CT_PP_DEFINED))
   {
   ...
   {
      LOG_FMT(LCASTS, "%s(%d): text is '%s', orig line is %zu, orig col is %zu, type is %s\n",
              __func__, __LINE__, pc->GetLogText(), pc->GetOrigLine(), pc->GetOrigCol(), get_token_name(pc->GetType()));

      if (  pc->Is(E_Token::CT_WORD)
         || (  last->Is(E_Token::CT_ANGLE_CLOSE)
            && pc->Is(E_Token::CT_DC_MEMBER)))
      {
         word_count++;
      }
      else if (  pc->Is(E_Token::CT_DC_MEMBER)
              || pc->Is(E_Token::CT_MEMBER)
              || pc->Is(E_Token::CT_PP))
      {
   ...
   if (  pc->IsNullChunk()
      || pc->IsNot(E_Token::CT_PAREN_CLOSE)
      || prev->Is(E_Token::CT_OC_CLASS))
   {
   ...
   if (  last->Is(E_Token::CT_STAR)
      || last->Is(E_Token::CT_CARET)
      || last->Is(E_Token::CT_PTR_TYPE)
      || last->Is(E_Token::CT_TYPE)
      || (  last->Is(E_Token::CT_ANGLE_CLOSE)
         && (  language_is_set(lang_flag_e::LANG_OC)
            || language_is_set(lang_flag_e::LANG_JAVA)
            || language_is_set(lang_flag_e::LANG_CS)
   ...
   else if (count == 1)
   {
      ...
      if (  (last->Len() > 3)
         ...
         && (last->GetText()[last->Len() - 1] == 't'))
      {
         detail = " -- '_t'";
      }
      else if (is_ucase_str(last->GetLogText(), last->Len()))
      {
         detail = " -- upper case";
      }
      ...
      } while (after->Is(E_Token::CT_PAREN_OPEN));
      ...
      if (pc->IsPointerOperator())
      {
         ...
         if (  after->Is(E_Token::CT_NUMBER_FP)
            || after->Is(E_Token::CT_NUMBER)
            || after->Is(E_Token::CT_STRING)
            || doubtful_cast)
         {
            nope = true;
         }
      ...
      {
         // (uint8_t)+1 or (foo)+1
         if (  (  after->IsNot(E_Token::CT_NUMBER)
               && after->IsNot(E_Token::CT_NUMBER_FP))
            || doubtful_cast)
         {
            nope = true;
         }
      }
      else if (  pc->IsNot(E_Token::CT_NUMBER_FP)
              && pc->IsNot(E_Token::CT_NUMBER)
              && pc->IsNot(E_Token::CT_WORD)
              && pc->IsNot(E_Token::CT_THIS)
              && pc->IsNot(E_Token::CT_TYPE)
              && pc->IsNot(E_Token::CT_PAREN_OPEN)
              && pc->IsNot(E_Token::CT_STRING)
              && pc->IsNot(E_Token::CT_DECLTYPE)
              && pc->IsNot(E_Token::CT_SIZEOF)
              && pc->GetParentType() != E_Token::CT_SIZEOF
              && pc->IsNot(E_Token::CT_FUNC_CALL)
              && pc->IsNot(E_Token::CT_FUNC_CALL_USER)
              && pc->IsNot(E_Token::CT_FUNCTION)
      ...
      if (nope)
      {
         LOG_FMT(LCASTS, "%s(%d):  -- not a cast - text '%s' followed by type %s\n",
                 __func__, __LINE__, pc->GetLogText(), get_token_name(after->GetType()));
         return;
      }
   }
   ...
   if (  pc->IsSemicolon()
      || pc->Is(E_Token::CT_COMMA)
      || pc->Is(E_Token::CT_BOOL)               // Issue #2151
      || pc->IsParenClose())
   {
   ...
   start->SetParentType(E_Token::CT_C_CAST);
   paren_close->SetParentType(E_Token::CT_C_CAST);
   ...
   for (pc = first;
        ...
        pc = pc->GetNextNcNnl())
   {
      pc->SetParentType(E_Token::CT_C_CAST);
      make_type(pc);
      LOG_FMT(LCASTS, " %s", pc->GetLogText());
   }
...
void fix_fcn_def_params(Chunk const *start)
{
   ...
   LOG_FMT(LFCNP, "%s(%d): text '%s', type is %s, on orig line %zu, level is %zu\n",
           __func__, __LINE__, start->GetLogText(), get_token_name(start->GetType()), start->GetOrigLine(), start->GetLevel());
   ...
   while (pc->IsNotNullChunk())
   {
      ...
      LOG_FMT(LFCNP, "%s(%d): %s, text '%s' on orig line %zu, level %zu\n",
              ...
              pc->GetLogText(), pc->GetOrigLine(), pc->GetLevel());
      ...
      else if (pc->Is(E_Token::CT_TYPE_WRAP))
      {
         cs.Push_Back(pc);
      }
              ...
              || pc->Is(E_Token::CT_ASSIGN))
      {
         ...
         if (pc->Is(E_Token::CT_ASSIGN))
         {
            ...
            pc->SetParentType(E_Token::CT_FUNC_PROTO);
         }
      }
      pc = pc->GetNextNcNnl();
   }
...
void fix_type_cast(Chunk const *start)
{
   ...
   if (  pc->IsNullChunk()
      || pc->IsNot(E_Token::CT_ANGLE_OPEN))
   {
      return;
   }
   pc = pc->GetNextNcNnl();
         ...
         && pc->GetLevel() >= start->GetLevel())
   {
         ...
         && pc->Is(E_Token::CT_ANGLE_CLOSE))
      {
         ...
         if (pc->IsString("("))
         {
            set_paren_parent(pc, E_Token::CT_TYPE_CAST);
         }
         return;
      }
      make_type(pc);
...
void fix_typedef(Chunk const *start)
{
        ...
        ; next = next->GetNextNcNnl(E_Scope::PREPROC))
   {
      ...
      if (start->GetLevel() == next->GetLevel())
      {
         if (next->IsSemicolon())
         {
            next->SetParentType(E_Token::CT_TYPEDEF);
            break;
         }
   ...
   if (  last_op->IsNotNullChunk()
      && !(  language_is_set(lang_flag_e::LANG_OC)
          && last_op->GetParentType() == E_Token::CT_ENUM))
   {
      flag_parens(last_op, PCF_NONE, E_Token::CT_FPAREN_OPEN, E_Token::CT_TYPEDEF, false);
      ...
      else
      {
         ...
         the_type->SetType(E_Token::CT_FUNC_TYPE);
      }
      the_type->SetParentType(E_Token::CT_TYPEDEF);

      LOG_FMT(LTYPEDEF, "%s(%d): fcn typedef text '%s', on orig line %zu\n",
              __func__, __LINE__, the_type->GetLogText(), the_type->GetOrigLine());
      ...
      if (options::align_typedef_func() != 0)
      {
         LOG_FMT(LTYPEDEF, "%s(%d):  -- align anchor on text %s, @ orig line %zu, orig col %zu\n",
                 __func__, __LINE__, the_type->GetLogText(), the_type->GetOrigLine(), the_type->GetOrigCol());
   ...
   if (  after->IsNot(E_Token::CT_ENUM)
      && after->IsNot(E_Token::CT_STRUCT)
      && after->IsNot(E_Token::CT_UNION))
   {
   ...
   if (next->Is(E_Token::CT_BRACE_OPEN))
   {
      ...
      Chunk *br_c = next->GetNextType(E_Token::CT_BRACE_CLOSE, next->GetLevel(), E_Scope::PREPROC);
      ...
      if (br_c->IsNotNullChunk())
      {
         ...
         if (tag == E_Token::CT_ENUM)
         {
            flag_series(after, br_c, PCF_IN_ENUM);
         }
         else if (tag == E_Token::CT_STRUCT)
         {
   ...
   if (the_type->IsNotNullChunk())
   {
      LOG_FMT(LTYPEDEF, "%s(%d): %s typedef text %s, on orig line %zu\n",
              __func__, __LINE__, get_token_name(after->GetType()), the_type->GetLogText(),
...
Chunk *fix_variable_definition(Chunk *start)
{
         ...
         || pc->IsPointerOperator())
   {
      ...
      LOG_FMT(LFVD, "%s(%d):   2:text '%s', type is %s\n",
              __func__, __LINE__, pc->GetLogText(), get_token_name(pc->GetType()));
      ...
      LOG_FMT(LFVD, "%s(%d):   4:text '%s', type is %s\n",
              __func__, __LINE__, pc->GetLogText(), get_token_name(pc->GetType()));
      ...
      if (language_is_set(lang_flag_e::LANG_JAVA))
      {
         ...
         if (pc->IsNotNullChunk())
         {
            LOG_FMT(LFVD, "%s(%d):   5:text '%s', type is %s\n", __func__, __LINE__, pc->GetLogText(), get_token_name(pc->GetType()));
         }
         else
         {
   ...
   if (end->Is(E_Token::CT_FUNC_CTOR_VAR))                // Issue #3010
   {
      return(end);
   }

   if (  cs.Len() == 1
      && end->Is(E_Token::CT_BRACE_OPEN)
      && end->GetParentType() == E_Token::CT_BRACED_INIT_LIST)
   {
      cs.Get(0)->m_pc->SetType(E_Token::CT_TYPE);
   }

   // Function defs are handled elsewhere
   if (  (cs.Len() <= 1)
      || end->Is(E_Token::CT_FUNC_DEF)
      || end->Is(E_Token::CT_FUNC_PROTO)
      || end->Is(E_Token::CT_FUNC_CLASS_DEF)
      || end->Is(E_Token::CT_FUNC_CLASS_PROTO)
      || end->Is(E_Token::CT_OPERATOR))
   {
         ...
         || (cs.Get(cs.Len() - 2)->m_pc->GetType() == E_Token::CT_DC_MEMBER)))
   {
      ...
      while (idx > 0)
      {
         ...
         if (  tmp_pc->IsNot(E_Token::CT_DC_MEMBER)
            && tmp_pc->IsNot(E_Token::CT_MEMBER))
         {
            break;
         }
         idx--;
         ...
         if (  tmp_pc->IsNot(E_Token::CT_WORD)
            && tmp_pc->IsNot(E_Token::CT_TYPE))
         {
            break;
         }
         make_type(tmp_pc);
   ...
   LOG_FMT(LFVD, "%s(%d): ref_idx(%d) is '%s'\n", __func__, __LINE__, ref_idx, tmp_pc->GetLogText());
   ...
   for (size_t idxForCs = 0; idxForCs < cs.Len() - 1; idxForCs++)
   {
      ...
      LOG_FMT(LFVD2, " text is '%s', type is %s", tmp_pc->GetLogText(), get_token_name(tmp_pc->GetType()));
   }
...
void mark_cpp_constructor(Chunk *pc)
{
   ...
   if (  tmp->Is(E_Token::CT_INV)
      || tmp->Is(E_Token::CT_DESTRUCTOR))
   {
      tmp->SetType(E_Token::CT_DESTRUCTOR);
      pc->SetParentType(E_Token::CT_DESTRUCTOR);
      is_destr = true;
   }
   ...
   if (!paren_open->IsString("("))
   {
      LOG_FMT(LWARN, "%s:%zu Expected '(', got: [%s]\n",
              ...
              paren_open->GetLogText());
      return;
   }
   // Mark parameters
   fix_fcn_def_params(paren_open);
   after = flag_parens(paren_open, PCF_IN_FCN_CALL, E_Token::CT_FPAREN_OPEN, E_Token::CT_FUNC_CLASS_PROTO, false);

   LOG_FMT(LFTOR, "%s(%d): text '%s'\n", __func__, __LINE__, after->GetLogText());
         ...
         && !tmp->IsSemicolon())
   {
      ...
      if (tmp->Is(E_Token::CT_BRACE_OPEN))
      {
         if (!tmp->TestFlags(PCF_IN_STRUCT))                              // Issue #4248
         {
            ...
            Chunk *closing = tmp->GetNextType(E_Token::CT_BRACE_CLOSE, tmp->GetLevel());
            LOG_FMT(LFTOR, "%s(%d): closing is '%s', orig line is %zu, orig col is %zu\n",
                    __func__, __LINE__, closing->GetLogText(), closing->GetOrigLine(), closing->GetOrigCol());
            tmp = closing;
         }
      }
      ...
      if (  tmp->IsString(":")
         && tmp->GetLevel() == paren_open->GetLevel())
      {
         tmp->SetType(E_Token::CT_CONSTR_COLON);
         hit_colon = true;
      }
         ...
         && tmp->GetLevel() == paren_open->GetLevel())
      {
         ...
         if (  var->Is(E_Token::CT_TYPE)
            || var->Is(E_Token::CT_WORD))
         {
            var->SetType(E_Token::CT_FUNC_CTOR_VAR);
            flag_parens(tmp, PCF_IN_FCN_CALL, E_Token::CT_FPAREN_OPEN, E_Token::CT_FUNC_CTOR_VAR, false);
         }
      }
   }
   ...
   if (tmp->Is(E_Token::CT_BRACE_OPEN))
   {
      set_paren_parent(paren_open, E_Token::CT_FUNC_CLASS_DEF);
      set_paren_parent(tmp, E_Token::CT_FUNC_CLASS_DEF);
...
void mark_define_expressions()
{
   ...
   while (pc->IsNotNullChunk())
   {
      if (!in_define)
      {
         if (  pc->Is(E_Token::CT_PP_DEFINE)
            || pc->Is(E_Token::CT_PP_IF)
            || pc->Is(E_Token::CT_PP_ELSE))
         {
            in_define = true;
            first     = true;
         }
...
void mark_exec_sql(Chunk const *pc)
{
   ...
   // Change E_Token::CT_WORD to E_Token::CT_SQL_WORD
   ...
   if (  pc->IsNot(E_Token::CT_SQL_BEGIN)
      || tmp->IsNullChunk()
      || tmp->IsNot(E_Token::CT_SEMICOLON))
   {
      return;
   }

   for (tmp = tmp->GetNext();
        tmp->IsNotNullChunk() && tmp->IsNot(E_Token::CT_SQL_END);
        tmp = tmp->GetNext())
   {
...
void mark_function_return_type(Chunk const *fname, Chunk *start, E_Token parent_type)
{
   ...
   if (pc->IsNotNullChunk())
   {
      ...
      LOG_FMT(LFCNR, "%s(%d): (backwards) return type for '%s' @ orig line is %zu, orig col is %zu\n",
              __func__, __LINE__, fname->GetLogText(), fname->GetOrigLine(), fname->GetOrigCol());
      ...
      while (pc->IsNotNullChunk())
      {
         ...
         if (  (  !pc->IsTypeDefinition()
               && pc->IsNot(E_Token::CT_OPERATOR)
               && pc->IsNot(E_Token::CT_WORD)
               && pc->IsNot(E_Token::CT_ADDR))
            || pc->TestFlags(PCF_IN_PREPROC))
         {
            break;
         }
      ...
      while (pc->IsNotNullChunk())
      {
         ...
         //template angles should keep parent type E_Token::CT_TEMPLATE
         if (pc->Is(E_Token::CT_ANGLE_OPEN))
         {
            pc = pc->GetNextType(E_Token::CT_ANGLE_CLOSE, pc->GetLevel());
...
void mark_function(Chunk *pc)
{
   ...
   if (pc->GetParentType() == E_Token::CT_OPERATOR)
   {
      ...
      {
         LOG_FMT(LFCN, "%s(%d): (4) SET TO E_Token::CT_FUNC_CALL: orig line %zu, orig col %zu, text '%s'\n",
                 __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
      ...
      if (language_is_set(lang_flag_e::LANG_CPP))
      {
         ...
         while ((tmp = tmp->GetPrevNcNnlNi())->IsNotNullChunk()) // Issue #2279
         {
               ...
               && !pc->TestFlags(PCF_IN_PREPROC))               // Issue #2703
            {
               ...
               LOG_FMT(LFCN, "%s(%d): (5) SET TO E_Token::CT_FUNC_CALL: orig line is %zu, orig col is %zu, text '%s'\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
               pc->SetType(E_Token::CT_FUNC_CALL);
               break;
            }
            ...
            if (tmp->Is(E_Token::CT_ASSIGN))
            {
               LOG_FMT(LFCN, "%s(%d): (6) SET TO E_Token::CT_FUNC_CALL: orig line is %zu, orig col is %zu, text '%s'\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
               pc->SetType(E_Token::CT_FUNC_CALL);
               break;
            }
            ...
            {
               LOG_FMT(LFCN, "%s(%d): (7) SET TO E_Token::CT_FUNC_DEF: orig line is %zu, orig col is %zu, text '%s'\n",
                       __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
            ...
            if (tmp->Is(E_Token::CT_BRACE_OPEN))
            {
               if (tmp->GetParentType() == E_Token::CT_FUNC_DEF)
               {
                  LOG_FMT(LFCN, "%s(%d): (8) SET TO E_Token::CT_FUNC_CALL: orig line is %zu, orig col is %zu, text '%s'\n",
                          __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
                  pc->SetType(E_Token::CT_FUNC_CALL);
               }

               if (  tmp->GetParentType() == E_Token::CT_CLASS
                  || tmp->GetParentType() == E_Token::CT_STRUCT)
               {
                  LOG_FMT(LFCN, "%s(%d): (9) SET TO E_Token::CT_FUNC_DEF: orig line is %zu, orig col is %zu, text '%s'\n",
                          __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
                  pc->SetType(E_Token::CT_FUNC_DEF);
               }
               break;
   ...
   LOG_FMT(LFCN, "   level %zu, brace level %zu, next->text '%s', next->type %s, next->level is %zu\n",
           ...
           next->GetLogText(), get_token_name(next->GetType()), next->GetLevel());
   ...
   if (pc->TestFlags(PCF_IN_CONST_ARGS))
   {
      ...
      flag_parens(next, PCF_NONE, E_Token::CT_FPAREN_OPEN, pc->GetType(), true);
      return;
   }
      ...
      && tmp->IsString("("))
   {
      ...
      while (tmp1->IsNotNullChunk())
      {
         ...
         if (  !tmp1->IsWord()
            || tmp2->IsNot(E_Token::CT_DC_MEMBER))
         {
            break;
         }
            ...
            || tmp2->Is(E_Token::CT_WORD)))
      {
         if (tmp2->IsNotNullChunk())
         {
            LOG_FMT(LFCN, "%s(%d): orig line is %zu, orig col is %zu, function variable '%s', changing '%s' into a type\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), tmp2->GetLogText(), pc->GetLogText());
            tmp2->SetType(E_Token::CT_FUNC_VAR);
            flag_parens(paren_open, PCF_NONE, E_Token::CT_PAREN_OPEN, E_Token::CT_FUNC_VAR, false);
         ...
         else
         {
            ...
            flag_parens(paren_open, PCF_NONE, E_Token::CT_PAREN_OPEN, E_Token::CT_FUNC_TYPE, false);
         }
         pc->SetType(E_Token::CT_TYPE);
         tmp1->SetType(E_Token::CT_PTR_TYPE);
         ...
         flag_parens(tmp, PCF_NONE, E_Token::CT_FPAREN_OPEN, E_Token::CT_FUNC_PROTO, false);
   ...
   if (pc->Is(E_Token::CT_FUNCTION))
   {
      ...
      if (temp->IsNotNullChunk())
      {
         LOG_FMT(LFCN, "%s(%d): assignment found, orig line is %zu, orig col is %zu, text '%s'\n",
                 __func__, __LINE__, temp->GetOrigLine(), temp->GetOrigCol(), temp->GetLogText());
         LOG_FMT(LFCN, "%s(%d): (10) SET TO E_Token::CT_FUNC_CALL: orig line is %zu, orig col is %zu, text '%s'",
                 __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
         pc->SetType(E_Token::CT_FUNC_CALL);
      }
      else
      {
         LOG_FMT(LFCN, "%s(%d): (11) SET TO %s: orig line is %zu, orig col is %zu, text '%s'",
                 __func__, __LINE__, (pc->GetParentType() == E_Token::CT_OPERATOR) ? "E_Token::CT_FUNC_DEF" : "E_Token::CT_FUNC_CALL",
                 pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
         pc->SetType((pc->GetParentType() == E_Token::CT_OPERATOR) ? E_Token::CT_FUNC_DEF : E_Token::CT_FUNC_CALL);
      }
   }
   ...
   if (prev->IsNotNullChunk())
   {
      LOG_FMT(LFCN, "%s(%d): prev->text is '%s', orig line is %zu, orig col is %zu, type is %s\n",
              __func__, __LINE__, prev->GetLogText(), prev->GetOrigLine(), prev->GetOrigCol(), get_token_name(prev->GetType()));
   }
            ...
            || prev->Is(E_Token::CT_DC_MEMBER))))
   {
      ...
      if (prev->Is(E_Token::CT_INV))
      {
         ...
         prev->SetType(E_Token::CT_DESTRUCTOR);
         pc->SetType(E_Token::CT_FUNC_CLASS_DEF);
      ...
      if (prev->Is(E_Token::CT_DC_MEMBER))
      {
         ...
         {
            if (pc->GetText().equals(prev->GetText()))
            {
               LOG_FMT(LFCN, "%s(%d): text is '%s', orig line is %zu, orig col is %zu, type is %s\n",
                       __func__, __LINE__, pc->GetLogText(), pc->GetOrigLine(), pc->GetOrigCol(),
                       get_token_name(prev->GetType()));
               pc->SetType(E_Token::CT_FUNC_CLASS_DEF);
      ...
      && !pc->TestFlags(PCF_IN_ARRAY_ASSIGN))
   {
      ...
      else
      {
         LOG_FMT(LFCN, "%s(%d): Checking func call: prev->text '%s', prev->GetType() is %s\n",
                 __func__, __LINE__, prev->GetLogText(), get_token_name(prev->GetType()));
      }
      ...
      while (prev->IsNotNullChunk())
      {
         ...
         if (pc->GetParentType() == E_Token::CT_FIXED)
         {
            isa_def = true;
         }
         ...
         if (  prev->Is(E_Token::CT_FPAREN_CLOSE)
            && prev->GetParentType() == E_Token::CT_ATTRIBUTE)
         {
            prev = skip_attribute_prev(prev);
            continue;
         }
         ...
         if (  prev->Is(E_Token::CT_PAREN_CLOSE)
            && prev->GetParentType() == E_Token::CT_D_CAST)
         {
         ...
         if (prev->GetParentType() == E_Token::CT_DECLSPEC)  // Issue 1289
         {
            ...
            if (prev->Is(E_Token::CT_DECLSPEC))
            {
               if (prev->IsNotNullChunk())
               {
                  prev = prev->GetPrev();
         ...
         // but one of the preceding tokens is a E_Token::CT_MEMBER than this is not a
         // fcn def, issue #1466
         if (  isa_def
            ...
            || prev->Is(E_Token::CT_MEMBER))
         {
                  ...
                  || prev->Is(E_Token::CT_MEMBER))
            {
               ...
               // Issue #1112
               // clarification: this will skip the E_Token::CT_WORD, E_Token::CT_TYPE or E_Token::CT_THIS landing on either
               // another E_Token::CT_DC_MEMBER or E_Token::CT_MEMBER or a token that indicates the context of the
               // token in question; therefore, exit loop when not a E_Token::CT_DC_MEMBER or E_Token::CT_MEMBER
               prev = prev->GetPrevNcNnlNpp();
         ...
         if (  prev->IsNot(E_Token::CT_OPERATOR)
            && prev->IsNot(E_Token::CT_TSQUARE)
            && prev->IsNot(E_Token::CT_ANGLE_CLOSE)
            ...
            && !(prev->IsParenClose() && prev->GetParentType() == E_Token::CT_DECLTYPE))
         {
            ...
            if (  prev->Is(E_Token::CT_ARITH)
               || prev->Is(E_Token::CT_SHIFT)
               || prev->Is(E_Token::CT_ASSIGN)
               || prev->Is(E_Token::CT_COMMA)
               || (  prev->Is(E_Token::CT_STRING)
                  && prev->GetParentType() != E_Token::CT_EXTERN)  // fixes issue 1259
               || prev->Is(E_Token::CT_STRING_MULTI)
               || prev->Is(E_Token::CT_NUMBER)
               || prev->Is(E_Token::CT_NUMBER_FP)
               || prev->Is(E_Token::CT_FPAREN_OPEN)) // issue #1464
            {
               isa_def = false;
            }
            break;
      ...
      if (prev->IsParenClose())
      {
         ...
         if (preproc->Is(E_Token::CT_PREPROC))
         {
            ...
            if (preproc->GetNextNcNnl()->Is(E_Token::CT_PP_ELSE))
            {
               do
               {
                  preproc = preproc->GetPrevNcNnlNi();      // Issue #2279

                  if (preproc->Is(E_Token::CT_PP_IF))
                  {
                     preproc = preproc->GetPrevNcNnlNi();   // Issue #2279
      ...
      if (  isa_def
         ...
         && (  (  prev->IsParenClose()
               && prev->GetParentType() != E_Token::CT_D_CAST
               && prev->GetParentType() != E_Token::CT_MACRO_OPEN  // Issue #2726
               && prev->GetParentType() != E_Token::CT_MACRO_CLOSE
               && prev->GetParentType() != E_Token::CT_DECLTYPE
               && prev->GetParentType() != E_Token::CT_MACRO_NO_INDENT
               && prev->GetParentType() != E_Token::CT_MACRO_NO_FMT_ARGS)
            || prev->Is(E_Token::CT_ASSIGN)
            || prev->Is(E_Token::CT_CO_RETURN)
            || prev->Is(E_Token::CT_CO_AWAIT)
            || prev->Is(E_Token::CT_CO_YIELD)
      ...
      if (isa_def)
      {
         LOG_FMT(LFCN, "%s(%d): pc is '%s', orig line is %zu, orig col is %zu, type is %s\n",
                 __func__, __LINE__, pc->GetLogText(), pc->GetOrigLine(), pc->GetOrigCol(), get_token_name(pc->GetType()));
         LOG_FMT(LFCN, "%s(%d): (12) SET TO E_Token::CT_FUNC_DEF: orig line is %zu, orig col is %zu, text '%s'\n",
                 __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
         pc->SetType(E_Token::CT_FUNC_DEF);
         ...
         for (tmp = prev; tmp->IsNotNullChunk() && tmp != pc; tmp = tmp->GetNextNcNnlNpp())
         {
            LOG_FMT(LFCN, "%s(%d): text is '%s', type is %s\n",
                    __func__, __LINE__, tmp->GetLogText(), get_token_name(tmp->GetType()));
            make_type(tmp);
         }
      }
   }

   if (pc->IsNot(E_Token::CT_FUNC_DEF))
   {
      LOG_FMT(LFCN, "%s(%d):  Detected type %s, text is '%s', on orig line %zu, orig col %zu\n",
              __func__, __LINE__, get_token_name(pc->GetType()),
              pc->GetLogText(), pc->GetOrigLine(), pc->GetOrigCol());

      tmp = flag_parens(next, PCF_IN_FCN_CALL, E_Token::CT_FPAREN_OPEN, E_Token::CT_FUNC_CALL, false);

      if (  tmp->IsNotNullChunk()
         && tmp->Is(E_Token::CT_BRACE_OPEN)
         && tmp->GetParentType() != E_Token::CT_DOUBLE_BRACE)
      {
   ...
   // Issue # 1403, 2152
   if (paren_open->GetPrev()->Is(E_Token::CT_FUNC_CTOR_VAR))
   {
      flag_parens(paren_open, PCF_IN_FCN_CTOR, E_Token::CT_FPAREN_OPEN, pc->GetType(), false);
   }
   else
   {
      // see also Issue #2103
      ...
      while (a->IsNotNullChunk())
      {
         LOG_FMT(LFCN, "%s(%d): orig line is %zu, orig col is %zu, text is '%s', type is %s, parent type is %s\n",
                 __func__, __LINE__, a->GetOrigLine(), a->GetOrigCol(), a->GetLogText(),
                 get_token_name(a->GetType()), get_token_name(a->GetParentType()));
         log_pcf_flags(LFCN, a->GetFlags());

         if (  a->Is(E_Token::CT_ARITH)
            && (strcmp(a->GetLogText(), "&") == 0))
         {
            a->SetType(E_Token::CT_BYREF);
         }
   ...
   if (pc->Is(E_Token::CT_FUNC_CTOR_VAR))
   {
      pc->SetFlagBits(PCF_VAR_1ST_DEF);
      return;
   }

   if (next->Is(E_Token::CT_TSQUARE))
   {
   ...
   if (pc->Is(E_Token::CT_FUNC_DEF))
   {
      ...
      while (  tmp->IsNotNullChunk()
            && tmp->IsNot(E_Token::CT_BRACE_OPEN))
      {
         LOG_FMT(LFCN, "%s(%d): (13) SET TO E_Token::CT_FUNC_DEF: orig line is %zu, orig col is %zu, text '%s'\n",
                 __func__, __LINE__, tmp->GetOrigLine(), tmp->GetOrigCol(), tmp->GetLogText());
         tmp->SetParentType(E_Token::CT_FUNC_DEF);
      ...
      if (tmp->Is(E_Token::CT_BRACE_OPEN))
      {
         LOG_FMT(LFCN, "%s(%d): (14) SET TO E_Token::CT_FUNC_DEF: orig line is %zu, orig col is %zu, text '%s'\n",
                 __func__, __LINE__, tmp->GetOrigLine(), tmp->GetOrigCol(), tmp->GetLogText());
         tmp->SetParentType(E_Token::CT_FUNC_DEF);
         ...
         if (tmp->IsNotNullChunk())
         {
            LOG_FMT(LFCN, "%s(%d): (15) SET TO E_Token::CT_FUNC_DEF: orig line is %zu, orig col is %zu, text '%s'\n",
                    __func__, __LINE__, tmp->GetOrigLine(), tmp->GetOrigCol(), tmp->GetLogText());
            tmp->SetParentType(E_Token::CT_FUNC_DEF);
         }
      }
...
bool mark_function_type(Chunk *pc)
{
   ...
   LOG_FMT(LFTYPE, "%s(%d): varcnk: text '%s', type %s, @ orig line %zu:, orig col %zu\n",
           __func__, __LINE__, varcnk->GetLogText(), get_token_name(varcnk->GetType()),
   ...
   LOG_FMT(LFTYPE, "%s(%d): apo: text is '%s', type is %s, @ orig line is %zu:, orig col is %zu\n",
           __func__, __LINE__, apo->GetLogText(), get_token_name(apo->GetType()),
   ...
   LOG_FMT(LFTYPE, "%s(%d): apc: text is '%s', type is %s, @ orig line is %zu:, orig col is %zu\n",
           __func__, __LINE__, apc->GetLogText(), get_token_name(apc->GetType()),
   ...
   LOG_FMT(LFTYPE, "%s(%d): aft: text is '%s', type is %s, @ orig line is %zu:, orig col is %zu\n",
           __func__, __LINE__, aft->GetLogText(), get_token_name(aft->GetType()),
           aft->GetOrigLine(), aft->GetOrigCol());

   if (aft->Is(E_Token::CT_BRACE_OPEN))
   {
      pt = E_Token::CT_FUNC_DEF;
   }
   else if (  aft->Is(E_Token::CT_SEMICOLON)
           || aft->Is(E_Token::CT_ASSIGN)
           || aft->Is(E_Token::CT_COMMA)                       // Issue #3259
           || aft->Is(E_Token::CT_FPAREN_CLOSE))               // Issue #3259
   {
      pt = E_Token::CT_FUNC_PROTO;
   }
   else
   {
   ...
   ptp = pc->TestFlags(PCF_IN_TYPEDEF) ? E_Token::CT_FUNC_TYPE : E_Token::CT_FUNC_VAR;
   ...
   while ((tmp = tmp->GetPrevNcNnlNi())->IsNotNullChunk()) // Issue #2279
   {
      ...
      LOG_FMT(LFTYPE, "%s(%d):  -- type is %s, %s on orig line %zu, orig col is %zu",             // Issue #3259
              __func__, __LINE__,
              get_token_name(tmp->GetType()), tmp->GetLogText(),
   ...
   {
      ptrcnk->SetType(E_Token::CT_PTR_TYPE);
   }
   ...
   if (!anon)
   {
      ...
      {
         varcnk->SetType(E_Token::CT_FUNC_TYPE);   // Issue #3402
      }
      else
      {
         varcnk->SetType(E_Token::CT_FUNC_VAR);
         varcnk->SetFlagBits(PCF_VAR_1ST_DEF);
      }
   }
   pc->SetType(E_Token::CT_TPAREN_CLOSE);
   ...
   flag_parens(apo, PCF_IN_FCN_DEF, E_Token::CT_NONE, pt, false);

   if (aft->IsSemicolon())
   {
      aft->SetParentType(aft->TestFlags(PCF_IN_TYPEDEF) ? E_Token::CT_TYPEDEF : E_Token::CT_FUNC_VAR);
   }
   else if (aft->Is(E_Token::CT_BRACE_OPEN))
   {
      flag_parens(aft, PCF_NONE, E_Token::CT_NONE, pt, false);
   }
   ...
   while ((tmp = tmp->GetPrevNcNnlNi())->IsNotNullChunk()) // Issue #2279
   {
      ...
      LOG_FMT(LFTYPE, "%s(%d):  ++ unbekannt: '%d'\n",
              __func__, __LINE__, *tmp->GetText().GetLogText());
      ...
      if (tmp->IsParenOpen())                          // 3259 ??
      {
         ...
         tmp->SetType(E_Token::CT_TPAREN_OPEN);
         tmp->SetParentType(ptp);

         tmp = tmp->GetPrevNcNnlNi();   // Issue #2279

         if (  tmp->Is(E_Token::CT_FUNCTION)
            || tmp->Is(E_Token::CT_FUNC_CALL)
            || tmp->Is(E_Token::CT_FUNC_CALL_USER)
            || tmp->Is(E_Token::CT_FUNC_DEF)
            || tmp->Is(E_Token::CT_FUNC_PROTO))
         {
...
nogo_exit:
   ...
   if (tmp->IsParenOpen())
   {
      ...
      flag_parens(tmp, PCF_IN_FCN_DEF, E_Token::CT_FPAREN_OPEN, E_Token::CT_FUNC_CALL, false);
   }
   return(false);
} // mark_function_type
      ...
      if (after->IsString("("))
      {
         if (angle_close->TestFlags(PCF_IN_FCN_CALL))
         {
            LOG_FMT(LTEMPFUNC, "%s(%d): marking '%s' in line %zu as a FUNC_CALL\n",
                    __func__, __LINE__, pc->GetLogText(), pc->GetOrigLine());
            LOG_FMT(LFCN, "%s(%d): (16) SET TO E_Token::CT_FUNC_CALL: orig line is %zu, orig col is %zu, text '%s'\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
            pc->SetType(E_Token::CT_FUNC_CALL);
            flag_parens(after, PCF_IN_FCN_CALL, E_Token::CT_FPAREN_OPEN, E_Token::CT_FUNC_CALL, false);
         }
         else
         {
            /*
            ...
            LOG_FMT(LTEMPFUNC, "%s(%d): marking '%s' in line %zu as a FUNC_CALL 2\n",
                    __func__, __LINE__, pc->GetLogText(), pc->GetOrigLine());
            // its a function!!!
            LOG_FMT(LFCN, "%s(%d): (17) SET TO E_Token::CT_FUNC_CALL: orig line is %zu, orig col is %zu, text '%s'\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText());
            pc->SetType(E_Token::CT_FUNC_CALL);
            mark_function(pc);
         }
      }
      else if (after->Is(E_Token::CT_WORD))
      {
         // its a type!
         pc->SetType(E_Token::CT_TYPE);
...
Chunk *mark_variable_definition(Chunk *start)
{
   ...
   while (go_on(pc, start))
   {
      ...
      else if (pc->Is(E_Token::CT_COLON))
      {
...
void mark_variable_stack(ChunkStack &cs, log_sev_t sev)
{
   ...
   if (  var_name->IsNotNullChunk()
      ...
      && var_name->GetPrev()->GetType() == E_Token::CT_DC_MEMBER)
   {
   ...
   if (var_name->IsNotNullChunk())
   {
      ...
      {
         if (  word_type->Is(E_Token::CT_WORD)
            || word_type->Is(E_Token::CT_TYPE))
         {
            LOG_FMT(LFCNP, "%s(%d): parameter on orig line %zu, orig col %zu: <%s> as TYPE\n",
                    __func__, __LINE__, var_name->GetOrigLine(), var_name->GetOrigCol(), word_type->GetLogText());
            word_type->SetType(E_Token::CT_TYPE);
            word_type->SetFlagBits(PCF_VAR_TYPE);
         }
         word_cnt++;
      }

      if (var_name->Is(E_Token::CT_WORD))
      {
         if (word_cnt > 0)
         {
            LOG_FMT(LFCNP, "%s(%d): parameter on orig line %zu, orig col %zu: <%s> as VAR\n",
                    __func__, __LINE__, var_name->GetOrigLine(), var_name->GetOrigCol(), var_name->GetLogText());
         ...
         else
         {
            LOG_FMT(LFCNP, "%s(%d): parameter on orig line %zu, orig col %zu: <%s> as TYPE\n",
                    __func__, __LINE__, var_name->GetOrigLine(), var_name->GetOrigCol(), var_name->GetLogText());
            var_name->SetType(E_Token::CT_TYPE);
...
PcfFlags mark_where_chunk(Chunk *pc, E_Token parent_type, PcfFlags flags)
{
   ...
   if (pc->Is(E_Token::CT_WHERE))
   {
      pc->SetType(E_Token::CT_WHERE_SPEC);
      pc->SetParentType(parent_type);
   ...
   else if (flags.test(PCF_IN_WHERE_SPEC))
   {
      if (pc->IsString(":"))
      {
         pc->SetType(E_Token::CT_WHERE_COLON);
