...
auto lambda2 = [x = std::move(value)]() mutable -> int&& {
		       return std::move(x);
	       };
auto lambda3 = [](int&& x, std::tuple<int&&, std::tuple<int&&, std::vector<std::string&&> > >&&) {
		       return x;
	       };
...
inline T&& T::operator+(T&& other) && {
	return std::move(*this);
}
inline bool T::operator==(const T& other) const && {
	return true;
}
inline auto T::operator%(T&& other) &&->T&& {
	return std::move(*this);
}
...
MyClass4&& MyClass4::getValue() && {
	return std::move(*this);
}
MyClass4&& MyClass4::process(MyClass4&& other) && {
...
ns2::Inner2&& ns2::Inner2::move() && {
	return std::move(*this);
}
MyClass4&& MyClass4::moveNoexcept() && noexcept {
	return std::move(*this);
}
MyClass4&& MyClass4::create(MyClass4&& src) {
	return std::move(src);
}
MyClass4&& operator+(MyClass4&& lhs, MyClass4&& rhs) {
	return std::move(lhs);
}
